...
async function main() {
  ...
  for (const person of persons) {
    try {
      ...
      console.log(`  -> Sending to ml-service-v2 for ArcFace extraction...`);
      const mlResponse = await axios.post('http://localhost:8001/v2/embed', form, {
        headers: {
          ...form.getHeaders(),
          'X-Api-Key': 'HKfZ0TP2Vei9Vtx88rPckAG6b4mZSSS5qqkxUKK6ib0SQSmrPQ_Wt2A2n1XTgotA'
        }
      });
      ...
      console.error(`  -> ERROR processing ${person.fullName}:`, err.message || err);
      if (err.response) {
