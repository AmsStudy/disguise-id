  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-[#151c2c] border border-white/5 rounded-xl p-12 flex flex-col items-center justify-center text-gray-400">
        ...
        <p className="text-lg">No review history found.</p>
        ...
        <table className="w-full text-left text-sm text-gray-300">
          <thead className="text-xs text-gray-400 uppercase bg-[#1e2638]">
            <tr>
              ...
              <th className="px-4 py-3 font-medium">Status / Decision</th>
              <th className="px-4 py-3 font-medium">Original Candidate</th>
                ...
                <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium mr-2 ${
                      item.status === 'COMPLETED' ? 'bg-green-500/10 text-green-400' : 'bg-gray-500/10 text-gray-400'
                    }`}>
                      {item.status}
                    </span>
                    {item.decision && (
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                        item.decision === 'CONFIRMED' ? 'bg-blue-500/10 text-blue-400' : 
                        item.decision === 'REJECTED' ? 'bg-red-500/10 text-red-400' :
