...
export default function MlV2PromotionsPage() {
  ...
  const [modalState, setModalState] = useState<{ isOpen: boolean; promotionId?: string | null; queueItem?: MlV2PromotionQueueItem | null; }>({ isOpen: false });
  ...
  if (user && !ALLOWED_ROLES.includes(user.role)) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', textAlign: 'center', padding: '48px 24px' }}>
        ...
        <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.5)', maxWidth: '360px', marginBottom: '24px' }}>You do not have permission to view ML V2 Promotions.</p>
  ...
  const handlePromote = (reviewId: string, payload: PromoteReviewPayload) => {
    if (!promoteMutation.isPending) promoteMutation.mutate({ reviewId, payload });
  };
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
            ...
            <h1 style={{ fontSize: '22px', fontWeight: 700, color: 'white', letterSpacing: '-0.02em', margin: 0 }}>Promotions</h1>
          </div>
          <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)', margin: 0 }}>Escalate confirmed operator reviews into explicit promotions.</p>
        </div>
        <button
